classdef NWBItem < bot.item.abstract.Item
    %An Item with one or more associated NWB files, 
    
    %% USER INTERFACE - Properties
        
    properties (Dependent, SetAccess=protected)
        nwbIsCached (1,:) logical % true if NWB file corresponding to this item is already cached
        nwbLocalFile (:,1) string
    end    
    ...
    %% STATIC METHODS
    methods (Static)
        function nwbNumFiles = zstNWBNumFiles()
            nwbNumFiles = bot.item.abstract.N.NWB_NUM_FILES;
        end
    end
